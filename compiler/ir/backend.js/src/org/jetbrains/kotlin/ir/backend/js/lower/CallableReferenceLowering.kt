...
class CallableReferenceLowering(val context: JsIrBackendContext) : FileLoweringPass {
    ...
    private fun buildGetFunction(declaration: IrFunction, reference: IrCallableReference, getterName: String): IrSimpleFunction {
        ...
        val closureParamsSize = (callableType as? IrSimpleType)?.arguments?.dropLast(1)?.size ?: 0 // drop return type
//        val closureParams = callableType.toKotlinType().arguments.dropLast(1) // drop return type
        var kFunctionValueParamsCount = closureParamsSize
        ...
        return JsIrBuilder.buildFunction(refGetSymbol).apply {
            ...
            for (i in 0 until getterValueParameters.size) {
                ...
                // TODO
                valueParameters += IrValueParameterImpl(p.startOffset, p.endOffset, p.origin, refGetSymbol.descriptor.valueParameters[i], p.type, p.varargElementType)
            }
        }
    }
    ...
    private fun buildClosureFunction(
        ...
        val returnType = declaration.descriptor.returnType?.toIrType()!!
        refClosureSymbol.initialize(valueParameters = closureParamDescriptors, returnType = returnType)

        val closureFunction = JsIrBuilder.buildFunction(refClosureSymbol).apply {
            this.returnType = returnType
        }

        for (it in closureParamSymbols) {
            // TODO hack
            closureFunction.valueParameters += JsIrBuilder.buildValueParameter(it, type = if (it.isBound) it.owner.type else context.irBuiltIns.anyType)
        }
